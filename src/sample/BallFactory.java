...
/**
 * Created by Sadiqur Rahman on 5/21/2016.
 */
public class BallFactory {
    ...
    public BallFactory(Group root, Scene scene) {
        ...
        final URL resource1 = getClass().getResource("start.wav");
        final URL resource2 = getClass().getResource("colide.wav");

        start = new AudioClip(resource1.toString());
        ballcolide = new AudioClip(resource2.toString());
        ...
        double r = 21;
        double variable;
        ball = new DrawBall[16];             /*1st 8 solidball including 8ball,last 8 stripball including cue ball   */
    ...
    public void setVelocityofCueball() {
        ...
        if ((turn.getTurn() && controllstick.getInitial_v() > 1 && !isSetVelocity1)) {
            ...
            //stripeball[7].setVx(5*Math.cos(Math.toRadians(stripeball[7].getAngle())));
            //stripeball[7].setVy(5*Math.sin(Math.toRadians(stripeball[7].getAngle())));
            ...
            System.out.println("setting when my turn");
        }
        ...
        if ((!turn.getTurn() && getIni_v() > 1 && !isSetVelocity2)) {
            ...
            solidballsize = ballStorage.solidballsize;
            stripeballsize = ballStorage.stripeballsize;
            System.out.println("setting when not my turn");
        }
    }
    ...
    public void controllVelocityofCueball() {
        if (isSetVelocity1) {
            ...
            if (isVelocityZero()) {
                ...
                System.out.println("velocity is 0 when my turn");
        ...
        if (isSetVelocity2) {
            ...
            if (isVelocityZero()) {
                ...
                System.out.println("velocity is 0 when not my turn");
    ...
    public void detectWallCollision() {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            //System.out.println(stripeball[7].getAx());
        }
    }
            ...
            for (int j = i + 1; j < 16; j++) {
                ...
                if (distSquared <= 20 * 20) {
                    ...
                    if (dotProduct > 0) {
                      ...
                      /* A.setAx((collisionWeightA * xCollision)+A.getAx());
                        A.setAy(collisionWeightA * yCollision+A.getAy());
                        B.setAx(B.getAx()- collisionWeightB * xCollision);
                        B.setAy(B.getAy()- collisionWeightB * yCollision);
*/
